...
-(void)setScores:(NSInteger)scores
{
    ...
    _scores = scores > self.numberOfStars ? self.numberOfStars : scores;
    _scores = scores < 0 ? 0 : scores;
    
    if ([self.delegate respondsToSelector:@selector(starRateView:scroesDidChange:)]) {
        [self.delegate starRateView:self scroesDidChange: _scores];
    }
